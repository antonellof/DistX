...
docker-stop:
	docker stop vectx && docker rm vectx
...
docker-clean:
	docker stop vectx 2>/dev/null || true
	docker rm vectx 2>/dev/null || true
	docker rmi antonellofratepietro/vectx:latest 2>/dev/null || true
	docker volume rm vectx_storage 2>/dev/null || true
