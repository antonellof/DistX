...
```python
import grpc
from distx_pb2_grpc import DistXStub

channel = grpc.insecure_channel('localhost:6334')
stub = DistXStub(channel)
```

### Create Collection

```python
from distx_pb2 import CreateCollectionRequest, VectorConfig
...
```python
from distx_pb2 import UpsertPointsRequest, Point, PointId, Vector
...
```python
from distx_pb2 import SearchPointsRequest, Vector
...
```python
from distx_pb2 import GetPointRequest, PointId
...
```python
from distx_pb2 import DeletePointRequest, PointId
...
```python
from distx_pb2 import ListCollectionsRequest
...
The gRPC API uses Protocol Buffers. See `lib/api/proto/distx.proto` for the complete schema.
