...
DistX delivers **comparable performance to Qdrant** while providing full API compatibility. Both systems offer high-performance vector search with SIMD optimizations.

## Test Environment

- **DistX**: v0.2.3, Docker deployment
...
```bash
# Run full comparison (DistX vs Qdrant vs Redis)
...
```bash
# Start DistX
docker run -d --name distx -p 6333:6333 -p 6334:6334 distx/distx
...
- Performance is comparable between DistX and Qdrant
