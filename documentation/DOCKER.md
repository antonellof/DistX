...
This guide covers how to run DistX using Docker for quick deployment without compilation.
...
docker run -p 6333:6333 -p 6334:6334 \
    -v "$(pwd)/distx_storage:/qdrant/storage" \
    distx/distx:latest \
    distx --data-dir /qdrant/storage --http-port 6333 --grpc-port 6334 --log-level debug
```
...
DistX provides a high-performance gRPC API on port 6334.
...
grpcurl -plaintext -d '{
  ...
  "vector_dim": 4,
  "distance": "Cosine"
}' localhost:6334 distx.DistX/CreateCollection
...
}' localhost:6334 distx.DistX/UpsertPoints
...
grpcurl -plaintext -d '{
  ...
  "limit": 5
}' localhost:6334 distx.DistX/SearchPoints

# List collections
grpcurl -plaintext localhost:6334 distx.DistX/ListCollections
```
...
# python -m grpc_tools.protoc -I. --python_out=. --grpc_python_out=. distx.proto
...
# Create collection
response = stub.CreateCollection(distx_pb2.CreateCollectionRequest(
    name="my_collection",
    vector_dim=128,
    distance="Cosine"
))
...
response = stub.UpsertPoints(distx_pb2.UpsertPointsRequest(
...
# Search
response = stub.SearchPoints(distx_pb2.SearchPointsRequest(
    collection_name="my_collection",
    vector=[0.1] * 128,
    limit=10
))
...
```bash
docker run -v distx_data:/qdrant/storage distx/distx:latest
```
...
docker inspect --format='{{.State.Health.Status}}' distx
```
...
docker run -it --rm distx/distx:latest /bin/bash
```

### Permission issues

```bash
# The container runs as user 'distx' (UID 1000)
# Ensure mounted directories are accessible
chown -R 1000:1000 ./distx_storage
```
...
```bash
# Use different ports
docker run -p 8080:6333 -p 8081:6334 distx/distx:latest
```
