#!/usr/bin/env python3
"""
Comprehensive Performance Comparison: DistX vs Qdrant
...
def print_comparison_table(all_results: Dict[str, List[BenchmarkResult]]):
    ...
    if len([r for results in all_results.values() for r in results if r.ops_per_sec > 0]) > 0:
        print("\nRelative Performance (DistX vs Qdrant):")
        print("-" * 60)
        
        for op in operations:
            distx_result = next((r for r in all_results.get("DistX", []) if r.operation == op), None)
            qdrant_result = next((r for r in all_results.get("Qdrant", []) if r.operation == op), None)
            
            if distx_result and qdrant_result and distx_result.ops_per_sec > 0 and qdrant_result.ops_per_sec > 0:
                ratio = distx_result.ops_per_sec / qdrant_result.ops_per_sec
                if ratio >= 1:
                    emoji = "🚀" if ratio >= 1.5 else "✅"
                    print(f"  {op:<25} {emoji} DistX is {ratio:.2f}x faster")
                else:
                    print(f"  {op:<25} ⚠️  Qdrant is {1/ratio:.2f}x faster")


def main():
    parser = argparse.ArgumentParser(description="Comprehensive DistX vs Qdrant benchmark")
    ...
    parser.add_argument("--distx-url", type=str, default="http://localhost:6333", help="DistX URL")
    parser.add_argument("--qdrant-url", type=str, default="http://localhost:16333", help="Qdrant URL")
    parser.add_argument("--distx-only", action="store_true", help="Only benchmark DistX")
    ...
    print("DistX vs Qdrant - Full API Comparison")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    if not args.qdrant_only:
        distx = VectorDBBenchmark("DistX", args.distx_url)
        if distx.is_available():
            all_results["DistX"] = run_comprehensive_benchmark(distx, config)
        else:
            print(f"\n⚠️  DistX not available at {args.distx_url}")
    
    # Benchmark Qdrant
    if not args.distx_only:
    ...
    if all_results:
        ...
        print("  1. Start DistX:  docker run -p 6333:6333 distx:latest")
