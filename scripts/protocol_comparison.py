...
Compares vectX REST, vectX gRPC, Qdrant REST, Qdrant gRPC, and Redis
"""
import requests
import redis
import grpc
...
# Configuration
VECTX_REST_URL = "http://localhost:6333"
VECTX_GRPC_HOST = "localhost:6334"
...
# ============================================================================
# vectX REST Benchmark
# ============================================================================
def benchmark_vectx_rest(num_vectors: int, num_searches: int) -> Dict:
    """Benchmark vectX REST API"""
    name = "vectX REST"
    url = VECTX_REST_URL
    collection = "bench_vectx_rest"
...
# ============================================================================
# vectX gRPC Benchmark
# ============================================================================
def benchmark_vectx_grpc(num_vectors: int, num_searches: int) -> Dict:
    """Benchmark vectX gRPC API"""
    name = "vectX gRPC"
    
    try:
        # Generate Python gRPC stubs if needed
        proto_file = os.path.join(PROTO_DIR, "vectx.proto")
        ...
        # Connect
        channel = grpc.insecure_channel(VECTX_GRPC_HOST)
        stub = vectx_pb2_grpc.vectXStub(channel)
        
        collection = "bench_vectx_grpc"
        
        # Create collection
        try:
            stub.CreateCollection(vectx_pb2.CreateCollectionRequest(
                name=collection,
                vector_dim=DIM,
                distance="Cosine"
            ))
        ...
        for i in range(0, num_vectors, BATCH_SIZE):
            ...
            for j in range(i, batch_end):
                point = vectx_pb2.Point(
                    id_integer=j,
                    vector=generate_vector(DIM)
                )
                points.append(point)
            
            stub.UpsertPoints(vectx_pb2.UpsertPointsRequest(
        ...
        for _ in range(num_searches):
            query = generate_vector(DIM)
            start = time.time()
            stub.SearchPoints(vectx_pb2.SearchPointsRequest(
                collection_name=collection,
                vector=query,
                limit=10
            ))
...
def main():
    ...
    # vectX REST
    print("  Testing vectX REST...")
    results.append(benchmark_vectx_rest(num_vectors, num_searches))
    
    # vectX gRPC
    print("  Testing vectX gRPC...")
    results.append(benchmark_vectx_grpc(num_vectors, num_searches))
    ...
    vectx_rest = next((r for r in results if r["name"] == "vectX REST" and r.get("available")), None)
    vectx_grpc = next((r for r in results if r["name"] == "vectX gRPC" and r.get("available")), None)
    ...
    if vectx_rest and vectx_grpc:
        insert_speedup = vectx_grpc["insert_ops"] / vectx_rest["insert_ops"]
        search_speedup = vectx_grpc["search_ops"] / vectx_rest["search_ops"]
        print(f"\nvectX gRPC vs REST:")
    ...
    if vectx_grpc and qdrant_grpc:
        insert_ratio = vectx_grpc["insert_ops"] / qdrant_grpc["insert_ops"]
        search_ratio = vectx_grpc["search_ops"] / qdrant_grpc["search_ops"]
        print(f"\nvectX gRPC vs Qdrant gRPC:")
