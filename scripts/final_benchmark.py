#!/usr/bin/env python3
"""
Final Performance Comparison: DistX vs Qdrant vs Redis Stack
...
def run_benchmark(name: str, num_vectors: int, dim: int, num_searches: int, batch_size: int):
    ...
    try:
        ...
        requests.delete(f"{DISTX_URL}/collections/{collection}", timeout=5)
        requests.put(f"{DISTX_URL}/collections/{collection}", json={
        ...
        for i in range(0, num_vectors, batch_size):
            ...
            requests.put(f"{DISTX_URL}/collections/{collection}/points", json={"points": points}, timeout=60)
        ...
        for _ in range(num_searches):
            ...
            start = time.time()
            requests.post(f"{DISTX_URL}/collections/{collection}/points/search",
        ...
        requests.delete(f"{DISTX_URL}/collections/{collection}", timeout=5)
        print(f"  DistX:  Insert {results['distx']['insert_ops']:,.0f} ops/s | Search {results['distx']['search_ops']:,.0f} ops/s (p50: {results['distx']['p50']:.2f}ms)")
    except Exception as e:
        print(f"  DistX: Error - {e}")
...
def main():
    ...
    print("=" * 80)
    print(f"{'Dataset':<12} {'DistX':<15} {'Qdrant':<15} {'Redis':<15} {'DistX vs Qdrant':<15}")
    print("-" * 80)
    for name, res in all_results.items():
        distx = res.get("distx", {}).get("insert_ops", 0)
        qdrant = res.get("qdrant", {}).get("insert_ops", 0)
        redis_val = res.get("redis", {}).get("insert_ops", 0)
        ratio = f"{distx/qdrant:.2f}x" if qdrant else "N/A"
        print(f"{name:<12} {distx:<15,.0f} {qdrant:<15,.0f} {redis_val:<15,.0f} {ratio:<15}")
    ...
    print("-" * 80)
    for name, res in all_results.items():
        distx = res.get("distx", {}).get("search_ops", 0)
