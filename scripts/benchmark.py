#!/usr/bin/env python3
"""
Performance benchmark script for vectX
...
def check_server() -> bool:
    """Check if vectX server is running"""
    try:
        response = requests.get(f"{VECTX_URL}/healthz", timeout=2)
...
def delete_collection(name: str) -> bool:
    ...
    try:
        response = requests.delete(f"{VECTX_URL}/collections/{name}", timeout=5)
...
def benchmark_insert(collection: str, num_vectors: int, dim: int, batch_size: int = 100) -> dict:
    ...
    for i in range(0, num_vectors, batch_size):
        ...
        try:
            response = requests.put(
                f"{VECTX_URL}/collections/{collection}/points",
                json={"points": points},
                timeout=60
            )
    ...
    for _ in range(num_searches):
        ...
        try:
            response = requests.post(
                f"{VECTX_URL}/collections/{collection}/points/search",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="vectX Performance Benchmark")
    parser.add_argument("--url", default=VECTX_URL, help="vectX server URL")
    ...
    print("=" * 70)
    print("vectX Performance Benchmark")
    print("=" * 70)
    print(f"Server: {VECTX_URL}")
    print()
    
    # Check server
    if not check_server():
        print("ERROR: vectX server is not running!")
        print(f"Please start it with: cargo run --release")
        print(f"Expected at: {VECTX_URL}")
        sys.exit(1)
