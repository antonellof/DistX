#!/usr/bin/env python3
"""
Performance Comparison Benchmark: DistX vs Qdrant vs Redis Stack

Compares insert and search performance across three vector databases:
- DistX (our implementation)
...
QDRANT_URL = "http://localhost:6333"  # Same port as DistX, we'll run separately
...
def main():
    parser = argparse.ArgumentParser(description="Compare DistX vs Qdrant vs Redis Stack")
    ...
    parser.add_argument("--distx-port", type=int, default=6333, help="DistX port (when running separately)")
    ...
    # Note: DistX and Qdrant use the same default port (6333)
    ...
    try:
        ...
        elif "distx" in data.get("title", "").lower():
            print("Detected: DistX running on port 6333")
            benchmarks_to_run.append(("distx", DistXBenchmark("http://localhost:6333")))
    except:
        pass
    ...
    if not benchmarks_to_run:
        ...
        print("  1. Start DistX:  cargo run --release")
    ...
    print("  DistX:  cargo run --release (port 6333)")
