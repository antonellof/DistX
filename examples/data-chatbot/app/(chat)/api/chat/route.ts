...
const wrapToolWithLogging = <T extends (...args: any[]) => any>(name: string, tool: T): T => {
  const originalExecute = (tool as any).execute;
  if (originalExecute) {
    (tool as any).execute = async (...args: any[]) => {
      console.warn(`\n========================================`);
      console.warn(`[DistX TOOL CALL] ${name}`);
      console.warn(`Input: ${JSON.stringify(args[0], null, 2)}`);
      console.warn(`========================================`);
      const result = await originalExecute(...args);
      console.warn(`[DistX TOOL RESULT] ${name}: ${result.success ? 'SUCCESS' : 'FAILED'}`);
      ...
      execute: async ({ writer: dataStream }) => {
        ...
        const result = streamText({
          ...
          tools: {
            ...
            // DistX data query tools (with logging)
