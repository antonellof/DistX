...
// Visibility removed - all chats are private
...
// Visibility removed - all chats are private by default
...
export function Chat({
  ...
  initialVisibilityType: "public" | "private"; // Kept for compatibility but not used
  isReadonly: boolean;
  autoResume: boolean;
}) {
  ...
  const {
    ...
    transport: new DefaultChatTransport({
      ...
      prepareSendMessagesRequest(request) {
        ...
        // Send existing messages (except the last one if it's new) for context
        const existingMessages = isToolApprovalContinuation
          ? request.messages
        ...
        return {
          body: {
            ...
            ...(isToolApprovalContinuation
              ...
              : { message: lastMessage }),
            // Include existing messages for context (client loads from localStorage)
    ...
    onFinish: () => {
      // Messages are saved via useEffect when messages state changes
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      saveMessagesToStorage(messagesWithChatId);
      
      // Save/update chat in localStorage
      ...
      if (chat) {
        ...
        // Create new chat (always private)
        saveChatToStorage({
          id,
          title: "New chat",
