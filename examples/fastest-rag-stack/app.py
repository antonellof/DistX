...
from rag_code import EmbedData, vectXVDB, Retriever, RAG, extract_text_from_file
...
def get_vectx_status():
    """Check vectX connection and get existing collections."""
    try:
        ...
        print(f"[DEBUG] vectX connection error: {e}")
...
def get_documents_in_collection(collection_name):
    ...
    try:
        vectx_vdb = vectXVDB(collection_name=collection_name, batch_size=32, vector_dim=1536)
        vectx_vdb.define_client()
        return vectx_vdb.list_documents()
    except Exception as e:
...
def delete_document_from_collection(collection_name, document_name):
    ...
    try:
        vectx_vdb = vectXVDB(collection_name=collection_name, batch_size=32, vector_dim=1536)
        vectx_vdb.define_client()
        return vectx_vdb.delete_document(document_name)
    except Exception as e:
...
def load_existing_collection(collection_name):
    ...
    try:
        vectx_vdb = vectXVDB(collection_name=collection_name, batch_size=batch_size, vector_dim=1536)
        vectx_vdb.define_client()
        
        # Create embeddata for query embedding
        embeddata = EmbedData(batch_size=batch_size)
        
        retriever = Retriever(vector_db=vectx_vdb, embeddata=embeddata)
...
with st.sidebar:
    # Check vectX status and show existing collections
    vectx_status = get_vectx_status()
    
    if vectx_status["connected"]:
        col1, col2 = st.columns([3, 1])
        with col1:
            st.success("🟢 vectX Connected")
        with col2:
            if st.button("🔄", help="Refresh"):
                st.rerun()
        
        if vectx_status["collections"]:
            st.subheader("📚 Existing Collections")
            
            for coll in vectx_status["collections"]:
        ...
        st.error("🔴 vectX Not Connected")
        st.caption(f"Error: {vectx_status.get('error', 'Unknown')}")
        st.caption("Make sure vectX is running on port 6333")
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if file_key not in st.session_state.get('file_cache', {}):
                    ...
                    # Set up vector database (vectX) first
                    vectx_vdb = vectXVDB(collection_name=collection_name,
                                         batch_size=batch_size,
                                         vector_dim=1536)
                    vectx_vdb.define_client()
                    vectx_vdb.create_collection()
                    ...
                    for uploaded_file in uploaded_files:
                        ...
                        try:
                            with st.spinner(f"Processing {uploaded_file.name}..."):
                                ...
                                vectx_vdb.ingest_data(embeddata=embeddata, document_name=uploaded_file.name)
...
with col1:
    ...
    st.header(f"RAG Stack with vectX and OpenAI ({llm_model})")
...
if not st.session_state.get('file_cache'):
    vectx_status = get_vectx_status()
    if vectx_status["connected"] and vectx_status["collections"]:
